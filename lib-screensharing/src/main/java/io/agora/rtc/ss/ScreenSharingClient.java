...
//    }
    ...
    @TargetApi(21)
    public void start(Context context, String appId, String token, String channelName, int uid, VideoEncoderConfiguration vec, String useraccount, String room_id, String rtcToken) {
        if (mScreenShareSvc == null) {
            Log.d("sjkhdasd", "inside 1");
            ...
            intent.putExtra(Constant.USERACCOUNT, useraccount);
            ...
            intent.putExtra(Constant.ROOM_ID, room_id);
            intent.putExtra(Constant.RTC_TOKRN, rtcToken);
            context.bindService(intent, mScreenShareConn, Context.BIND_AUTO_CREATE);
        } else {
            Log.d("sjkhdasd", "inside 2");
            try {
                mScreenShareSvc.startShare();
            } catch (RemoteException e) {
                e.printStackTrace();
